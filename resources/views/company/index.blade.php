                            ...
                            <!-- Table Headings -->
                                ...
                                @foreach ($currentCompanies as $company)
                                    <tr>
                                        ...
                                        <td>
                                            <div>
                                                {!! Form::model($company, ['method' => 'GET', 'route' => ['company.edit', $company->companyId]]) !!}
                                                <button type="submit" class="btn btn-warning"><i class="fa fa-trash"></i></i> Edit </button>
                                                <a href="{!!URL::route('company.destroy', ['id' => $company->companyId])!!}" class="btn btn-danger"
                                                   onclick="return confirm('Are you sure about deleting the company?');">Delete</a>
                                                <a href="{!!URL::route('company.departments', ['id' => $company->companyId])!!}" class="btn btn-info">Departments</a>
                                                <a href="{!!URL::route('company.teams', ['id' => $company->companyId])!!}" class="btn btn-info">Teams</a>
                                                {!! Form::close() !!}
                                            </div>
                                        </td>
                                    </tr>
